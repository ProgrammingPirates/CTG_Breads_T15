
<div class="dropdown">
    <button class="btn btn-primary dropdown-toggle export-btns" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
        <?php echo e(__('Export')); ?>

    </button>
    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
        <a class="dropdown-item has-icon" href="" id="export_print">
            <span class="navi-icon">
                <i class="fa fa-print"></i>
            </span>
            <span class="navi-text ml-2"><?php echo e(__('Print')); ?></span>
        </a>

        <a class="dropdown-item has-icon" href="" id="export_copy">
            <span class="navi-icon">
                <i class="fa fa-copy"></i>
            </span>
            <span class="navi-text ml-2"><?php echo e(__('Copy')); ?></span>
        </a>

        <a class="dropdown-item has-icon" href="" id="export_excel">
            <span class="navi-icon">
                <i class="fa fa-file-excel"></i>
            </span>
            <span class="navi-text ml-2"><?php echo e(__('Excel')); ?></span>
        </a>

        <a class="dropdown-item has-icon" href="" id="export_csv">
            <span class="navi-icon">
                <i class="fa fa-file-csv"></i>
            </span>
            <span class="navi-text ml-2"><?php echo e(__('CSV')); ?></span>
        </a>

        <a class="dropdown-item has-icon" href="" id="export_pdf">
            <span class="navi-icon">
                <i class="fa fa-file-pdf"></i>
            </span>
            <span class="navi-text ml-2"><?php echo e(__('PDF')); ?></span>
        </a>
    </div>
</div>
<?php /**PATH C:\xampp\htdocs\doctro\resources\views/superAdmin/auth/exportButtons.blade.php ENDPATH**/ ?>