...
class UserApiController extends Controller
{
    ...
    public function apiSendNotification(request $request)
    {
        ...
        $this->oneSignalNoti($request->query('device_token'), auth()->user()->name);

        Config::set('onesignal.app_id', env('doctor_app_id'));
        Config::set('onesignal.rest_api_key', env('doctor_api_key'));
        Config::set('onesignal.user_auth_key', env('doctor_auth_key'));
        Config::set('onesignal.android_channel_id', 'e3df6c9d-c251-4d01-adf1-180e2f1a1e22');
        $note = OneSignal::sendNotificationToUser(
            "Some Message",
            ["aa61f9fb-827e-4849-a12b-368fdfcb85e7"],
            $url = null,
                $data = [
                    'name' => auth()->user()->name,
                    'id' => 1,
        ...
        return response()->json(['success' => true, 'message' => $note]);
